...
| 구분                   | 1차 모델 (Isolation Forest)               | 2차 모델 (RandomForest + SMOTE)         |
| :--------------------- | :---------------------------------------- | :-------------------------------------- |
| **학습 방식**          | 비지도 (Unsupervised, 정답 레이블 미사용) | **지도 (Supervised, 정답 레이블 사용)** |
| **데이터 불균형**      | 해소 안 함 (모델의 기본 성질에 의존)      | **SMOTE로 해소 (데이터 증강)**          |
| **핵심 전략**          | "이상한 놈 찾기" (정상 분포 이탈 탐지)    | **"사기/정상 패턴 학습" (정확한 분류)** |
| **Recall (탐지율)**    | 0.26 (26%)                                | **0.77 (77%)**                          |
| **Precision (정밀도)** | 0.26 (26%)                                | **0.85 (85%)**                          |
| **F1-Score (종합)**    | 0.26 (26%)                                | **0.81 (81%)**                          |
